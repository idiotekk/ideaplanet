...
input_dir = Path("/Users/zche//data/0xgenerator/bg_switch/inputs/")
output_dir = Path("/Users/zche//data/0xgenerator/bg_switch/outputs/") 
...
def bg_switch(file_name):
    ...
    is_black = (
        np.logical_or.reduce((
            frm.is_same_color(input_arr, color=frm.BLACK, tol=50),
            frm.is_same_color(input_arr, color=frm.WHITE, tol=100),
    ...
    all_neighbors_are_yellow = np.zeros(is_bg_color.shape)
    rr = 1
    count = 0
    for i in range(-rr, rr+1):
        for j in range(-rr, rr+1):
            all_neighbors_are_yellow += np.roll(is_bg_color, (i, j), (0, 1))
            count += 1
    all_neighbors_are_yellow  = (all_neighbors_are_yellow >= count)
